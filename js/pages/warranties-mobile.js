...
function warrantyCardHTML(w) {
  ...
  const expiryText = used
    ? `Used ${w.claimed_date ? shortDate(w.claimed_date) : ''}${w.claim_notes ? ` · ${escapeHTML(w.claim_notes)}` : ''}`
    : w.expiry_date
  ...
  return `
    <div class="m-card" data-id="${w.id}" style="${expired || used ? 'opacity:0.6' : ''};border-color:${borderColor}">
...
function render({ data, loading }) {
  ...
  const list = (warranties(data))
    .filter(w => !w.archived)
    .sort((a, b) => {
      // Active first sorted by expiry asc, then expired/used by expiry
      const aExp = a.expiry_date || '9999-99-99';
      const bExp = b.expiry_date || '9999-99-99';
      const aActive = warrantyStatus(a, today) === 'active';
      const bActive = warrantyStatus(b, today) === 'active';
      if (aActive && !bActive) return -1;
      if (!aActive && bActive) return 1;
      return aExp.localeCompare(bExp);
    });

  const active = list.filter(w => warrantyStatus(w, today) === 'active');
  const expired = list.filter(w => warrantyStatus(w, today) === 'expired');
  const used = list.filter(w => warrantyStatus(w, today) === 'used');
  ...
  const usedHTML = used.length
    ? `<div class="m-section-hdr">Used (${used.length})</div><div class="m-list">${used.map(w => warrantyCardHTML(w)).join('')}</div>`
    : '';

  page.innerHTML = summaryStripHTML(data) + activeHTML + usedHTML + expiredHTML;
}
