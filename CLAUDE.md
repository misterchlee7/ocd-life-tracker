...
- **Warranty used / consumed.** `claimed_date` (+ optional `claim_notes`) marks a warranty as redeemed — claiming it fulfils it, so it is no longer valid whatever `expiry_date` says. Derived by `warrantyStatus(w, today)` in derive.js → `used` | `expired` | `active` (`used` wins over `expired`); desktop + mobile both use it. UI: green "Used" badge, expiry date struck through, `Used <date> · <claim note>` sub-line, faded row (`tr.warranty-used`), excluded from active/expiring counts and from `warranty_expiry` attention items. Set via row menu → "✅ Mark warranty used" (own date + note modal, `openUseModal`; amountModal is numeric-only), reversible via "↩︎ Unmark used"; both fields also editable in the warranty form. Filters gain a "Show used" chip (default on). Mobile groups used rows under a "Used (n)" section, read-only as ever.
- **Accounts.** `data.accounts || []` — always access defensively. Read-mostly registry of financial accounts (institution + name, `account_type`, who, last4, APY, opened date). **No live balance tracking** — balances are optional manual snapshots (`snapshots: [{date, balance}]`, ascending, one per date; same-day re-snapshot replaces) added via "Add balance snapshot" (amountModal) in the row menu or the bulk "Update balances" flow. UI shows latest snapshot + age. Status `open`/`closed` (not `archived`) — closed rows faded via `.archived` class, kept for reference, toggled via row menu. **Marking closed writes a $0 snapshot** (same mutation) when the latest balance is non-zero, so the trend chart's aggregate stays honest. Summary cards: Open count (who breakdown), Snapshot total (+ coverage & oldest-snapshot age), Best APY, Closed count. Mobile is read-only cards (no actions).
- **Accounts balance trend chart** (desktop only, see `docs/decisions.md` for the full rationale). Panel between summary cards and filters; renders only when ≥1 open investment account (`brokerage`/`retirement`/`hsa`) exists. Hand-rolled inline SVG line chart — no chart libs. Scope chips (All investment / per type / single-account select) + range chips (3M/6M/YTD/1Y/All). Series math lives in derive.js: `snapshotAt`, `balanceSeries` (forward-fill; dots = real snapshots; synthetic carried points at window start/end), `seriesDelta` — covered in `tests/dates.test.js`. **Growth, not performance**: includes contributions, labeled as such; never call it "return". Closed accounts stay in the aggregate series (their history is real) but not in chips/select. Readout: current + Δ$ + Δ% since window start; per-account breakdown table when scope has ≥2 accounts. Hover = crosshair + tooltip (nearest point, "carried" for synthetic). Chart colors are tokens: `.cg-line`/`.cg-dot` use `--accent`, grid `--border` — light-dark() safe.
- **Bulk snapshot ("Update balances", accounts page header).** One modal, every open account (investment types first), each field optional — blank = skip, placeholder shows last balance. Filled fields become today-dated snapshots via shared `applySnapshot()` (same-day-replace) in ONE `state.mutate()` labeled `bulk snapshot: <names>`.
